# Function for plotting enrichment.

plotEnrichment <- function(data, allResultsProportions) {
  ...
  allGenes <- allGenes[which(allGenes$Gene %in% data$Gene),]
  ...
  for (level in unique(allGenes$Expression)) {
    ...
    for (mod in unique(c(controlGenes1$Mod.TF, Rgenes1$Mod.TF))) {
