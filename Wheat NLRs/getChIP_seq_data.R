...
getChIP_seq_data <- function() {
  ...
  ChIP_experiments <- as.data.frame(read.csv("Wheat NLRs/ChIP experiment SRA data.csv"))

  # Combine nextflow pipeline outputs into a single dataframe.
  nextflowOutput <- data.frame()
  
  for (file in list.files(path = "Wheat nextflow output", pattern = "Peaks.bed")) {
    
    # Rename file to include target modification/TF (if it has not been changed already).
    if (str_detect(file, "_SRR") == FALSE) {
      
      file.rename(paste("Wheat nextflow output/", file, sep = ""), 
                  paste("Wheat nextflow output/", ChIP_experiments[which(str_detect(ChIP_experiments$Sample.data, str_match(file,"^(SRR.*)_merged.*$")[,2])==TRUE), "Modification.TF"],
                        "_", file, sep = "")) 
    ...
    data <-  as.data.frame(import.bed(paste("Wheat nextflow output/", file, sep = "")))
