...
for (file in list.files(path = "Cut-and-Tag", pattern = "JTL60")) {

  # Merge all broad and narrow peaks datasets.
  data <- as.data.frame(import.bed(paste("Cut-and-Tag/", file, sep = "")))
  
  # Add data to 'nextflowOutput'.
  nextflowOutput <- rbind(nextflowOutput, data)
}

nextflowOutput <- nextflowOutput[-which(nextflowOutput$seqnames == "Mt" | nextflowOutput$seqnames == "Pt"),]
...
nextflowOutputBed <- GRanges(seqnames = nextflowOutput$seqnames, ranges = nextflowOutput$ranges)
...
for (geneSet in names(sampleGenes)) {
  for (level in c("No Expression", "Low Expression")) {
    ...
    addWorksheet(allProportions_wb, sheetName = "H3K27me3")
    addWorksheet(allFrequency_wb, sheetName = "H3K27me3")
    ...
    for (region in names(geneRegions)) {
      ...
      overlap <- bt.intersect(nextflowOutputBed, queryBed, wo = TRUE) 
      ...
      for (gene in unique(geneRegions[[region]]$Gene)) {
        ...
        if (gene %in% overlap$V11) {
          peakOverlaps <- overlap[overlap$V11==gene,]
          ...
          mergedOverlap <- rbind(mergedOverlap, data.frame(Gene = gene,
                                                           ...
                                                           overlap = sum(peakOverlaps$V12)/peakOverlaps$V9[1]))
    ...
    writeData(allProportions_wb, sheet = "H3K27me3", allProportions_data)
    writeData(allFrequency_wb, sheet = "H3K27me3", allFrequency_data)
    
    # Save workbook as an .xlsx file.
    saveWorkbook(allProportions_wb, paste("Cut-and-Tag/All proportions/JTL60_", geneSet, "_", level, ".xlsx", sep = ""), overwrite = TRUE) 
    saveWorkbook(allFrequency_wb, paste("Cut-and-Tag/All frequencies/JTL60_", geneSet, "_", level, ".xlsx", sep = ""), overwrite = TRUE) 
  }
}
...
for (file in list.files("Cut-and-Tag/All proportions/", pattern = "JTL60")) {
  
  allProportions <- read.xlsx(paste("Cut-and-Tag/All proportions/", file, sep = ""), sheet = "H3K27me3")
  allProportions_results <- rbind(allProportions_results, allProportions)
}
for (file in list.files("Cut-and-Tag/All frequencies/", pattern = "JTL60")) {
  allFrequency <- read.xlsx(paste("Cut-and-Tag/All frequencies/", file, sep = ""), sheet = "H3K27me3")
...
pdf("Cut-and-Tag/Enrichment per region/JTL60 H3K27me3.pdf", width = 12, height = 5)
print(plot)
dev.off() 