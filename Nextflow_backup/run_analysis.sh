...
then
	...
	then
		for file in "${sample_data[@]}"; do if [ ! -f "$file"]; then get_data "$file"; fi; get_data_paired "$file"; done;
...
then
	if [ "$sequencing_type" = "single" ];
	then
		for file in "${sample_data[@]}" "${control_data[@]}"; do if [ ! -f "$file"]; then get_data "$file"; fi; get_data "$file"; done;
	...
	then
		for file in "${sample_data[@]}" "${control_data[@]}"; do if [ ! -f "$file"]; then get_data "$file"; fi; get_data_paired "$file"; done;
...
#cd ..;
rclone copy ~/nextflow-peakcaller/output OneDrive:Nextflow_backup